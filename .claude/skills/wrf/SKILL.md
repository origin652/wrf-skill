---
name: wrf
description: Orchestrate the end-to-end WRF workflow from natural-language intent or structured research spec through init, config, async data/WPS/run tasks, and status follow-up.
---
...
6. Runtime selection is config-driven: `hpc.runtime` controls `wrf-run` with `project|remote_run_dir|custom`, and `hpc.wps_runtime` controls `wrf-wps` with `project|remote_wps_dir|custom`.
