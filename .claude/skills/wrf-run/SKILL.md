...
6. Support substep-level execution with `--only` / `--from` for both local and HPC runs.
7. Use `scripts/wrf.py status`, `scripts/wrf.py logs`, and `scripts/wrf.py logs --substep <name>` to report queued/running/completed/failed/canceled progress.
8. When an HPC run is terminal, use `scripts/wrf.py collect` to pull logs and outputs back into local project state.
...
- If `--from wrf` is used on HPC, local `wrfinput_d*` and `wrfbdy_d01` must already exist before sync.
- Remote post-processing still runs after `wrf.exe` when the selected substeps include `wrf`.
