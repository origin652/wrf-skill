...
6. Use `scripts/wrf_task.py status` and `logs` to report queued/running/completed/failed/canceled.
...
```bash
python3 scripts/wrf_task.py logs --project-name demo --config config/wrf_env.json --lines 100
