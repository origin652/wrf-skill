...
5. If the task is an HPC task in terminal state and outputs are not yet reflected locally, use `scripts/wrf_task.py collect`.
6. Report:
...
```bash
python3 scripts/wrf_task.py logs --project-name demo --lines 100
```

Cancel an active task:
```bash
python3 scripts/wrf_task.py cancel --project-name demo --task-id <task-id>
```
