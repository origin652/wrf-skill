...
try:
    from download_gfs import build_manifest, download_manifest
...
def ensure_supported_source(source: str) -> None:
    if str(source).lower() != "gfs":
        raise NotImplementedError(
            f"wrf-data currently supports only GFS, received: {source}"
        )
...
def build_inventory(
    ...
    for request in manifest["requests"]:
        ...
        exists = local_path.exists() and size_bytes > 0
        record = deepcopy(request)
        record["local_path"] = posix_path(local_path)
        record["exists"] = exists
        record["size_bytes"] = size_bytes if exists else 0
...
def write_download_script(manifest: dict[str, Any], script_path: Path) -> None:
    ...
    if not missing_requests:
        lines.append('echo "All planned GFS files are already present."')
    else:
...
def prepare_data(
    ...
    *,
    ...
    ensure_supported_source(spec["data_source"])

    effective_interval = (
        interval_hours
        if interval_hours is not None
        else int(base_state["data_source"].get("interval_hours") or 3)
    )
    data_dir = Path(base_state["paths"]["data_dir"])
    manifest_path = data_dir / "data_manifest.json"
    download_script_path = data_dir / "download_gfs.sh"
    ...
    inventory = build_inventory(manifest, data_dir)
    ...
    print(f"wrf-data start project={project_name} total_requests={len(manifest['requests'])} missing={inventory['missing_count']}", flush=True)
    ...
    merged_manifest = merge_download_results(manifest, download_result)
    inventory = build_inventory(merged_manifest, data_dir)
    enriched_manifest = enrich_manifest(
        project_name,
        merged_manifest,
        inventory,
    ...
    append_log_lines(
        ...
        [
            "phase=finished",
            f"downloaded_count={download_result['summary']['downloaded_count']}",
            f"skipped_count={download_result['summary']['skipped_count']}",
            f"failed_count={download_result['summary']['failed_count']}",
    ...
    if not inventory["complete"]:
        ...
        save_project(state, project_json_path)
        raise RuntimeError("GFS download incomplete; see wrf-data.log and data_manifest.json")
